...
# MAXIMIZE TOKEN SAVINGS: Try local for these 4 categories first.
...
def main():
    ...
    for task in tasks:
        ...
        if answer is None:
            ...
            if not verify_answer(category, answer) and tier != "hard":
                answer = call_fireworks(tier_to_model["hard"], system_prompt, prompt, max_tokens)

        results.append({"task_id": task_id, "answer": answer})
        print(f"{task_id}: {category} -> {'Local' if used_local else 'Fireworks'}")
